...
**1. Delegation happy path (rung 3).** You in VC-A, all three bots idle. Run `/play` on **B3's** slash command.
Expect: B3 posts a handoff notice; B1 (first idle in list order) joins VC-A and posts now-playing **as itself**. Check the now-playing embed's author and avatar are B1's, and that B1's player buttons respond. This is the whole architecture in one test.

**2. Rung 1 continuity — highest consequence if wrong.** B1 already playing in VC-A, you in VC-A. Run `/play` on **B2**.
Expect: `in_user_vc` → B1, track appended to the existing queue, **no second bot joins**.
...
**5. Guards validate the chosen bot, not the receiver.** Deny **B1** `Connect` on VC-A via channel overwrite; leave B2/B3 permitted. You in VC-A, all idle, run `/play` on B2 (so B1 gets picked by list order).
Expect: `no_connect_permission` and **no handoff notice**. A handoff notice followed by silence means the wrong bot was validated — this is the specific correctness property the whole design exists for.

**6. Channel-visibility guard.** Deny **B1** `ViewChannel` on the text channel you're typing in, leave B2 permitted. Run `/play` on B2 with all bots idle.
Expect: a permission failure via B2's reply, not a silent hang.
...
**10. Cross-bot search collector.** `/search <query>` on B2 with B1 idle so B1 is chosen.
Expect: B1 posts the results and **B1's** collector accepts your selection.
...
## Operational risk that needs a decision before 2026-08-27
...
This is not a bug in this branch; it is a consequence of the intent loss meeting a pre-existing feature. The obvious fix is to allow slash commands in setup channels — drop the rejection, or gate it on `messageContentIntent`. It does not block this merge, and it must not be forgotten.
...
- `Context.delegated`'s `members.resolve` is cache-only. With `GuildMembers` dropped, it may often miss and fall back to the receiver's member object. The fallback prevents a crash; whether it defeats the fix's intent needs test 5 to tell.
- `scripts/add-interaction-locale-keys.js` and `scripts/invert-deprecation-notice.js` are one-shot migration scripts that have already run. Both are idempotent. Delete when convenient.
